...
export default function Professional04({ data, className }: TemplateProps) {
    ...
    switch (key) {
      case 'summary':
        return summary?.content ? (
          <div key={key} className="mb-5">
            <h2 className="text-sm font-bold text-blue-600 uppercase tracking-wider mb-2 border-b border-blue-200 pb-1">Summary</h2>
      ...
      case 'experience':
        return experience?.length > 0 ? (
          <div key={key} className="mb-5">
            <h2 className="text-sm font-bold text-blue-600 uppercase tracking-wider mb-2 border-b border-blue-200 pb-1">Experience</h2>
            {experience.map((exp) => (
                ...
                {exp.highlights?.length > 0 && (
                  <ul className="mt-1 space-y-0.5">
                    {exp.highlights.map((h, i) => (
                      <li key={i} className="text-xs text-gray-700 pl-3 relative before:content-['•'] before:absolute before:left-0 before:text-blue-500">{h}</li>
                    ))}
                  </ul>
                )}
              </div>
      ...
      case 'education':
        return education?.length > 0 ? (
          <div key={key} className="mb-5">
            <h2 className="text-sm font-bold text-blue-600 uppercase tracking-wider mb-2 border-b border-blue-200 pb-1">Education</h2>
            {education.map((edu) => (
      ...
      case 'projects':
        return projects?.length > 0 ? (
          <div key={key} className="mb-5">
            <h2 className="text-sm font-bold text-blue-600 uppercase tracking-wider mb-2 border-b border-blue-200 pb-1">Projects</h2>
            {projects.map((proj) => (
              <div key={proj.id} className="mb-3">
                <div className="flex justify-between items-start">
                  <h3 className="text-sm font-semibold text-gray-900">{proj.name}</h3>
                  {proj.url && <span className="text-xs text-blue-600 ml-2 truncate max-w-[120px]">{proj.url}</span>}
                </div>
    ...
    switch (key) {
      case 'skills':
        return skills?.length > 0 ? (
          <div key={key} className="mb-5">
            <h2 className="text-xs font-bold text-blue-600 uppercase tracking-wider mb-2 border-b border-blue-200 pb-1">Skills</h2>
            {skills.map((group, i) => (
                  ...
                  {group.items.map((item, j) => (
                    <div key={j}>
                      <div className="flex justify-between mb-0.5">
                        ...
                        <span className="text-[9px] text-gray-400">{60 + ((j * 17) % 40)}%</span>
                      </div>
                      <div className="w-full h-1.5 bg-gray-200 rounded-full">
                        <div className="h-full bg-blue-600 rounded-full" style={{ width: `${60 + ((j * 17) % 40)}%` }} />
                      </div>
                    </div>
                  ))}
                </div>
      ...
      case 'certifications':
        return certifications?.length > 0 ? (
          <div key={key} className="mb-5">
            <h2 className="text-xs font-bold text-blue-600 uppercase tracking-wider mb-2 border-b border-blue-200 pb-1">Certifications</h2>
            {certifications.map((cert) => (
              <div key={cert.id} className="mb-2">
                <h3 className="text-[10px] font-semibold text-gray-800">{cert.name}</h3>
                <p className="text-[9px] text-gray-500">{cert.issuer} &middot; {formatDate(cert.date)}</p>
              </div>
            ))}
          </div>
        ) : null;
  ...
  return (
    <div className={cn('w-[794px] min-h-[1123px] bg-white', className)}>
      ...
      <div className="bg-blue-600 text-white px-8 py-5">
        ...
        {personalInfo.title && <p className="text-sm text-blue-100 font-medium mt-0.5">{personalInfo.title}</p>}
        <div className="flex flex-wrap gap-x-4 gap-y-0.5 mt-2 text-xs text-blue-100">
      ...
      <div className="flex">
        ...
        {/* Sidebar - 40% */}
        <div className="w-[40%] bg-gray-50 p-6 border-l border-gray-200">
          {sectionOrder
            .filter((key) => key !== 'personalInfo')
